...
function* tilesGenerator(limits: [number, number, number, number], zoom: Zoom, metatile: number): Generator<Tile, undefined, undefined> {
  for (let y = limits[1]; y <= limits[3]; y++) {
    for (let x = limits[0]; x <= limits[2]; x++) {
      yield { x: x, y: y, z: zoom, metatile: metatile };
    }
  }
...
export function zoomShift(zoom: Zoom, referenceTileGrid: TileGrid, targetTileGrid: TileGrid): Zoom {
  ...
  targetTileGrid.wellKnownScaleSet.scaleDenominators.forEach((targetScaleDenominator, targetZoom) => {
